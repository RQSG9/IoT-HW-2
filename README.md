...
### EX 5-14
 - 예제 5-13에서 this 값을 변경해야하는 문제를 해결하기 위해 부분 적용 함수를 생성하는 사용자 정의 함수를 직접 구현
 - partial 호출 시 전달된 원본 함수와 초기 인자들을 기억
 - Array.prototype.slice.call()과 partialArgs.concat()을 통해 초기 인자와 새 인자를 합함
 - func.apply(this, ...)를 사용하여 this를 보존

### EX 5-15
 - 예제 5-14의 partial 함수를 유연하게 개선, 인자들을 원하는 임의의 위치에 미리 두고 나중에 빈 자리에 추가 인자를 받아 채워 실행
 - Object.defineProperty를 이용해 변경 불가능한 전역객체의 프로퍼티 _를 정의하고, 이를 나중에 채워질 인자를 위한 자리 표시자로 사용

### EX 5-16
 - 짧은 시간 동안 동일한 이벤트가 많이 발생할 경우, 이를 그룹화하여 단 한 번만 콜백 함수를 실행시키는 디바운스
 - 반환된 함수는 timeoutId 변수를 자신의 스코프 안에 기억, 이벤트가 발생할 때마다, 기존에 설정된 setTimeout을 clearTimeout으로 취소하고, 새로운 setTimeout을 설정하여 wait 시간 후에 func가 실행되도록 예약
 - 연속적으로 빠르게 발생하는 동안에는 func가 절대 실행되지 않음, 이벤트 발생이 wait 밀리초 동안 멈췄을 때만, 마지막으로 예약된 func가 한 번 실행

### EX 5-17
 - 여러 개의 인자를 받는 함수를, 각각 하나의 인자만 받는 함수의 연속된 호출로 변환하는 커링 기법
 - getMaxWith10은 10과 입력값을 비교하여 더 큰 값을, getMinWith10은 더 작은 값을 반환, 따라서 콘솔에는 순서대로 10, 25, 8, 10이 출력

### EX 5-18
 - 예제 5-17에서 소개된 커링 기법을 5개의 인자를 받는 함수에 적용할 수 있도록 확장
 - 5단계로 중첩된 함수 구조를 가지고 있어 가독성이 떨어짐
 - ES6에서는 화살표 함수를 써서 같은 내용을 한 줄에 표기 가능